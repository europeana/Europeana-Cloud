...
set -e
set -x
# /volume/cassandra_backup.sh -H /opt/apache-cassandra-2.1.8 -b sftp://ecloud:ecloud_admin@sftp:22/backup --yaml /opt/apache-cassandra-2.1.8/conf/cassandra.yaml -p cassandra -u cassandra -v backup
# /volume/cassandra_backup.sh -H /opt/apache-cassandra-2.1.8 -b sftp://ecloud:ecloud_admin@sftp:22/backup --yaml /opt/apache-cassandra-2.1.8/conf/cassandra.yaml -p cassandra -u cassandra  inventory
# /volume/cassandra_backup.sh -H /opt/apache-cassandra-2.1.8 -b sftp://ecloud:ecloud_admin@sftp:22/backup/backups/17c0598f9638/snpsht/2017-06-22_12-34 --yaml /opt/apache-cassandra-2.1.8/conf/cassandra.yaml -p cassandra -u cassandra  -k -d /var/lib/cassandra/backups restore
...
function validate() {
  ...
  if [ -z ${GCS_BUCKET} ]; then
      ...
      if ! $(${LFTP} -c "open ${GCS_BUCKET}; ls" &> /dev/null) ; then
  ...
  if [ ${ACTION} != "inventory" ]; then
    ...
    if [ ${ACTION} = "restore" ]; then
      GCS_LS=$(${LFTP} -c "open ${GCS_BUCKET}; ls" | sed 1d | sed 1d | head -n1 | awk -F " " '{print $9}')
...
function inventory() {
  ...
  if ! $(${LFTP} -c "open ${GCS_BUCKET}; ls /backup/backups/${HOSTNAME}/snpsht" 2>&1 &> /dev/null); then
      loginfo "No snapshots"
  else
    ${LFTP} -c "open ${GCS_BUCKET}; ls /backup/backups/${HOSTNAME}/snpsht" | sed 1d | sed 1d
  fi
  if [ -z $incremental_backups ] || [ $incremental_backups = false ]; then
    loginfo "Incremental Backups are not enabled for Cassandra"
  fi
  loginfo "Available Incremental Backups:"
  if ! $(${LFTP} -c "open ${GCS_BUCKET}; ls /backup/backups/${HOSTNAME}/incr" 2>&1 &> /dev/null); then
    loginfo "No incremental backups"
  else
    ${LFTP} -c "open ${GCS_BUCKET}; ls /backup/backups/${HOSTNAME}/incr" | sed 1d | sed 1d
  fi
}
...
function copy_to_sftp() {
  ...
  if ${DRY_RUN}; then
    ...
    ${LFTP} -c "open ${GCS_BUCKET}; mkdir -p -f /backup/backups/${HOSTNAME}/${SUFFIX}/${DATE}"
    if ${SPLIT_FILE}; then
...
function restore_split_from_gcs() {
  ...
  if ${DRY_RUN}; then
    ...
    ${LFTP} -c "open ${GCS_BUCKET}; mirror -R ${GCS_BUCKET}" "${COMPRESS_DIR}"
...
function restore_compressed_from_gcs() {
    if ${DRY_RUN}; then
      ...
      ${LFTP} -c "open ${GCS_BUCKET}; get ${GCS_BUCKET} -o ${COMPRESS_DIR}"
    fi
    restore_decompress
}
...
function restore_fix_perms() {
  ...
  if ${DRY_RUN}; then
    ...
    chown -R ${CASSANDRA_OG} ${1} #TODO decide
  fi
}
...
function stop_cassandra(){
      ...
      pkill -f org.apache.cassandra.service.CassandraDaemon # TODO refactor
...
while getopts 'a:b:BcCd:DfhH:iIjkl:LnN:p:rs:S:T:u:U:vwy:x:z' OPTION
do
  case $OPTION in
      a)
          HOSTNAME=${OPTARG}
...
CASSANDRA_OG="root:root" #modify this if you changed the system cassandra user and group
