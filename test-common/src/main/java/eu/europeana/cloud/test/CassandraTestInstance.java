...
public final class CassandraTestInstance {
  ...
  private CassandraTestInstance() {
    ...
    try {
      ...
      URL res = getClass().getClassLoader().getResource(CASSANDRA_CONFIG_FILE);
      File file = Paths.get(res.toURI()).toFile();
      EmbeddedCassandraServerHelper
          .startEmbeddedCassandra(file, "target/embeddedCassandra", CASSANDRA_STARTUP_TIMEOUT);
   ...
   *
   * @param hard if is true then empty tables are truncated (this slow down process because it
   * require disk flushes)
   */
  public static synchronized void truncateAllData(boolean hard) {
    ...
    for (String keyspaceName : keyspaceSessions.keySet()) {
      if (hard) {
        LOGGER.warn(
            "Truncating all tables! Operation is slow use CassandraTestInstance.truncateAllData" +
                "(false).");
  ...
  public static synchronized void print() {
    ...
    for (String keyspaceName : keyspaceSessions.keySet()) {
      ...
      for (Row r : rs.all()) {
        ...
        LOGGER.info("keyspace : {}, table : {}  have rows : {} ", keyspaceName, tableName,
            rows.getAvailableWithoutFetching());
      }
    }
  }
  ...
  private static final class TestRetryPolicy implements RetryPolicy {
    ...
    private RetryDecision getRetryDecision(ConsistencyLevel cl, int required, int actual,
        int nbRetry, double maxNbRetry) {
      if (actual >= required) {
        return RetryDecision.ignore();
