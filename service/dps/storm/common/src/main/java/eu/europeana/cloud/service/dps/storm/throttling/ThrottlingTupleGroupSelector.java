...
/**
 *  Class is responsible for generating grouping attribute used for throttling. This allows implements throttling on
 *  the bolt level. Special artificial attribute is generated so every tuple for given task is assigned to his group.
 *  Only limited number of attribute value is generated for given task based on MAXIMUM_PARALLELIZATION parameter.
 *  So tuples from the given task could only go to limited number of bolts. This enforce throttling that is global on
 *  topology level in distributed Storm environment, without external synchronization between bolts.
 */
