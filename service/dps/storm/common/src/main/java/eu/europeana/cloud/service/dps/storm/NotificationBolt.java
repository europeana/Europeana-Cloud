...
public class NotificationBolt extends BaseRichBolt {
	 ...
	 * 
	 * @param hosts
	 *            Cassandra hosts separated by comma (e.g.
	 ...
	 *            Cassandra password
	 * @param grouping
	 *            this bolt is connected to topology by fields grouping If true:
	...
	public void prepare(Map stormConf, TopologyContext tc, OutputCollector oc) {
		cassandra = new CassandraConnectionProvider(hosts, port, keyspaceName,
				userName, password);
	...
	private int storeBasicInfo(long taskId, Map<String, String> parameters) {
		...
		Insert insert = QueryBuilder.insertInto(BASIC_INFO_TABLE).values(
				BASIC_INFO_COLUMNS,
	...
	private void storeNotification(long taskId, Map<String, String> parameters) {
		...
		Insert insert = QueryBuilder.insertInto(NOTIFICATIONS_TABLE).values(
				NOTIFICATIONS_COLUMNS,
				new Object[] { taskId, topologyName, resource, state, infoText,
						additionalInfo });
