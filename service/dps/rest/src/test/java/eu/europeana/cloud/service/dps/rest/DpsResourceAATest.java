...
public class DpsResourceAATest extends AbstractSecurityTest {
    ...
    public void shouldThrowDpsTaskValidationExceptionOnSubmitTaskToIcTopologyWithMissingTaskSubmitterName() throws AccessDeniedOrTopologyDoesNotExistException, DpsTaskValidationException, TaskSubmissionException {
        ...
        try {
            //when
            topologyTasksResource.submitTask(IC_TASK, topologyName, URI_INFO, AUTH_HEADER_VALUE);
            fail();
        } catch (DpsTaskValidationException e) {
            //then
            assertThat(e.getMessage(), is("Expected parameter does not exist in dpsTask. Parameter name: TASK_SUBMITTER_NAME"));
    ...
    public void runtimeExceptionShouldBeThrownWhenMCSIsNotAvailable() throws MCSException, AccessDeniedOrTopologyDoesNotExistException, TaskSubmissionException, DpsTaskValidationException {
        ...
        topologiesResource.grantPermissionsToTopology(RONALDO, SAMPLE_TOPOLOGY_NAME);
        login(RONALDO, RONALD_PASSWORD);
        topologyTasksResource.submitTask(XSLT_TASK2, SAMPLE_TOPOLOGY_NAME, URI_INFO, AUTH_HEADER_VALUE);
    ...
    public void exceptionShouldBeThrownWhenTaskUrlIsMalformed() throws MCSException, AccessDeniedOrTopologyDoesNotExistException, TaskSubmissionException, DpsTaskValidationException {
        ...
        topologiesResource.grantPermissionsToTopology(RONALDO, SAMPLE_TOPOLOGY_NAME);
        login(RONALDO, RONALD_PASSWORD);
        topologyTasksResource.submitTask(XSLT_TASK_WITH_MALFORMED_URL, SAMPLE_TOPOLOGY_NAME, URI_INFO, AUTH_HEADER_VALUE);
    }
