...
public class InMemoryContentDAO {
	...
	public void getContent(String globalId, String schema, String version, String fileName, long rangeStart,
			long rangeEnd, OutputStream os) throws IOException, FileNotExistsException, WrongContentRangeException {
		...
		if (rangeStart != -1) {
			...
			data = Arrays.copyOfRange(data, (int) rangeStart,
					(int) ((rangeEnd == -1 || rangeEnd > data.length - 1) ? data.length - 1 : rangeEnd) + 1);
		}
		os.write(data);
	}
