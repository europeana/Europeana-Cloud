...
import eu.europeana.cloud.service.aas.authentication.UserImpl;
import eu.europeana.cloud.service.aas.authentication.User;
import eu.europeana.cloud.service.aas.authentication.UserDAO;
...
public class CassandraUserDAO  implements UserDAO {
	 ...
	 */
	public CassandraUserDAO(final DatabaseService databaseService) {
	 ...
	 * Used to create / retrieve / update <code>User</code> objects from ecloud's database. 
	 * 
	 * @param session session the <code>Session</code> to use for connectivity with Cassandra.
