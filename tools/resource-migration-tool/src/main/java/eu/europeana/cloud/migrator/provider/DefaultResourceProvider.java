...
public abstract class DefaultResourceProvider
    ...
    try {
      ...
      for (Iterator<Path> i = dirStream.iterator(); i.hasNext(); ) {
        ...
        if (Files.isDirectory(path)) {
          ...
          FilePaths providerPaths = getProviderPaths(Paths.get(location).toAbsolutePath().toString()
                                                          .replace(ResourceMigrator.WINDOWS_SEPARATOR,
                                                              ResourceMigrator.LINUX_SEPARATOR), providerId, paths);
  ...
  /**
   * Default implementation of determining filename from path. The result is either path without location, part of the specified
   * path after "/" or "\" character or path itself.
   *
  ...
  private List<FilePaths> split(FilePaths fp) {
    ...
    if (size <= DEFAULT_LIST_SIZE) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        filePath.getFullPaths().addAll(fp.getFullPaths().subList(i * DEFAULT_LIST_SIZE,
            (i + 1) * DEFAULT_LIST_SIZE < size ? (i + 1) * DEFAULT_LIST_SIZE : size));
        filePath.setIdentifier(String.valueOf(i * DEFAULT_LIST_SIZE) + "_" + String.valueOf(
            (i + 1) * DEFAULT_LIST_SIZE < size ? (i + 1) * DEFAULT_LIST_SIZE : size));
        result.add(filePath);
      }
      if (rest > 0) {
