...
public class ResourceMigrator {
  ...
  /**
   * FileProcessor is an interface that supplies functions for some processing on the original files before they are uploaded to
   * ECloud.
   */
  ...
  public ResourceMigrator(ResourceProvider resProvider, String dataProvidersMappingFile, String threadsCount,
      FileProcessorFactory fileProcessorFactory) throws IOException {
  ...
  public void verifyLocalIds() {
    ...
    try {
      ...
      for (Future<LocalIdVerificationResult> result : results) {
        ...
        logger.info(
            "Verification of local identifier part {} performed successfully. Verification time: {} sec. Number of not migrated identifiers: {}",
  ...
  public boolean grant(boolean simulate) {
    ...
    try {
      ...
      for (Future<PublicAccessGranterResult> result : results) {
        ...
        logger.info(
            "Public access granter part {} performed successfully. Granting time: {} sec. Number of not granted identifiers: {}",
  ...
  private void summarize(Map<String, List<FilePaths>> paths) {
    ...
    try {
      ...
      Files.write(dest, msg.getBytes(StandardCharsets.UTF_8), StandardOpenOption.CREATE, StandardOpenOption.APPEND,
  ...
  /**
   * Create representation for the specified provider and cloud identifier. Representation name is supplied by the resource
   * provider. If there is already a representation and it is persistent then we cannot add anything to this record. If there are
   * non persistent representations they will be removed. Please, be careful when calling this method.
   *
   ...
   *
   * @param path path to file, if location is remote it must have correct URI syntax, otherwise it must be a proper path in a
   * filesystem
  ...
  private boolean processProvider(String resourceProviderId, FilePaths providerPaths) {
    ...
    removeProcessedPaths(providerPaths.getIdentifier() != null ? providerPaths.getIdentifier() : resourceProviderId,
        providerPaths);
      ...
      for (; ; ) {
        ...
        if (!localId.equals(prevLocalId)) {
          if (prevLocalId != null && cloudIds.get(prevLocalId) != null && versionIds.get(prevLocalId) != null
              && resourceProvider.getFileCount(prevLocalId) == fileCount.get(prevLocalId)) {
            if (logger.isDebugEnabled()) {
            ...
            if (persistent != null) {
              ...
              saveProgress(fp.getIdentifier() != null ? fp.getIdentifier() : resourceProviderId,
                  processed.get(versionIds.get(prevLocalId)), false, null);
  ...
  private void saveProgress(String providerId, List<String> strings, boolean truncate, String prefix) {
    try {
      Path dest = FileSystems.getDefault()
                             .getPath(".", (prefix != null ? prefix : "") + providerId + ResourceMigrator.TEXT_EXTENSION);
      if (truncate) {
        // make copy
        ...
        Files.write(dest, strings, StandardCharsets.UTF_8, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING,
            StandardOpenOption.WRITE);
      } else {
        Files.write(dest, strings, StandardCharsets.UTF_8, StandardOpenOption.CREATE, StandardOpenOption.APPEND,
            StandardOpenOption.WRITE);
      }
    } catch (IOException e) {
  ...
  private boolean permitVersion(String cloudId, String version) {
    ...
    while (retries-- > 0) {
      try {
        ...
        logger.warn("Error processing HTTP request while granting permissions to version: " + version + " for record: " + cloudId
  ...
  public void verify() {
    ...
    try {
      ...
      for (Future<VerificationResult> result : results) {
        providerResult = result.get();
        logger.info(
            "Verification of provider {} performed successfully. Verification time: {} sec. Number of not migrated files: {}",
            providerResult.getProviderId(),
            providerResult.getTime(),
    ...
    try {
      for (; ; ) {
        try {
          ...
          List<Representation> representations = mcs.getRepresentations(cloudIds.get(localId),
              resourceProvider.getRepresentationName());
      ...
      saveProgress(providerPaths.getIdentifier() != null ? providerPaths.getIdentifier() : resourceProviderId, strings, true,
          "verify_");
    } finally {
      if (reader != null) {
        try {
      ...
      for (FilePaths fp : paths) {
        ...
        try {
          ...
          for (; ; ) {
            ...
            Files.write(dest, line.getBytes(StandardCharsets.UTF_8), StandardOpenOption.CREATE, StandardOpenOption.APPEND,
                StandardOpenOption.WRITE);
          }
        } catch (IOException e) {
  ...
  private class ProviderVerifier implements Callable<VerificationResult> {
    ...
    public VerificationResult call()
      ...
      if (split.equals(paths)) {
        ...
        try {
          ...
          for (Future<VerificationResult> result : results) {
            partResult = result.get();
            logger.info(
                "Verification of part {} ({}) performed successfully. Verification time: {}sec. Number of not migrated files: {}",
                partResult.getIdentifier(),
                partResult.getProviderId(),
                partResult.getTime(),
      ...
      return new VerificationResult(notMigrated, providerId, (float) (System.currentTimeMillis() - start) / (float) 1000,
          identifier);
    }
  }
