...
public interface ResourceProvider {
  ...
  /**
   * Get resource provider identifier as string. Resource provider is usually institution that has its directory within the
   * resource location. It is NOT the data provider used in ECloud however the same identifier may be used.
   *
  ...
  /**
   * Returns the data provider identifier used in ECloud. Usually it is the same for all ResourceProvider records and it's defined
  ...
  /**
   * Get basic information on data provider such as organisation name, address and others. See <code>DataProviderProperties</code>
   * for details.
   *
  ...
  /**
   * Return the default representation name of records added for this resource provider. The representation name is the same for
   * all records within one run of migration tool.
   *
  ...
  /**
   * Scan location in case it's local. Otherwise return an empty map. The returned map should associate provider identifier (it
   * can be determined from the path) with the list of files to add.
   *
  ...
  /**
   * Determine the number of files that should be added for the specified local identifier. It may be done using the mapping file
   * or any other way proper for the resource provider. When it is impossible to determine -1 is returned.
   *
  ...
  /**
   * Resource provider implementation may split the file paths that will be migrated into several parts. The way of splitting is
   * specific for a resource provider. The idea of splitting the file paths is to use many threads during the migration process.
   * For each FilePaths object there will be a separate thread.
   *
  ...
  /**
   * Mapping file can associate local record identifier with a set of paths to files. Internally mapping is stored in a reversed
   * form since the lookups are done using path to file and not the local identifier. This method returns reversed mapping.
   *
