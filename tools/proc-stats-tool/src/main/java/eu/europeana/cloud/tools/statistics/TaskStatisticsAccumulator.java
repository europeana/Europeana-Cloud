...
/**
 * Accumulator realise inc/dec algorithm of opened connections
 * Additionally it stores summary time (durations) for given number of opened connections (for average calculating)
 * Additionally accumulator recognizes situation that first info about some operation is ending operation [END]
 * so it increases number of opened connections by one from the beginning of analysing to found [END] line
 */
public class TaskStatisticsAccumulator {
    ...
    /**
     * Map keeps processing time for given number of opened connection
     * Opened connections is a kay to map
     * Summary time (stored as duration) is a value for given key
     * This map is needed only for counting average value
     */
    ...
    /**
     * Count average value of opened connections normalized by processing time
     * 5 connections by 100s is not this same like 5 connection by 10s!
     * Result is stored in statistics structure
