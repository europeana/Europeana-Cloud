...
public class V10_4__copy_data_to_new_tables implements JavaMigration {
  ...
  private void initStatements(Session session) {
    ...
    selectAssignmentsStatement = session.prepare(
        "select provider_dataset_id,cloud_id, schema_id, version_id, creation_date from data_set_assignments");
  ...
  public void migrate(Session session) throws Exception {
    ...
    while (hasNextRow(ri)) {
      ...
      //
      Bucket bucket = bucketsHandler.getCurrentBucket(DATA_SET_ASSIGNMENTS_BY_DATA_SET_TABLE_NAME,
