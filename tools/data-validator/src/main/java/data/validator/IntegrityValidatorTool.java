...
public class IntegrityValidatorTool {
  ...
  private static void executeIntegrityValidation(CommandLine cmd) throws Exception {
    ...
    CassandraConnectionProvider sourceCassandraConnectionProvider = getCassandraConnectionProviderFromConfiguration(SOURCE_HOSTS,
        SOURCE_PORT, SOURCE_KEYSPACE, SOURCE_USER_NAME, SOURCE_PASSWORD);
    CassandraConnectionProvider targetCassandraConnectionProvider = getCassandraConnectionProviderFromConfiguration(TARGET_HOSTS,
        TARGET_PORT, TARGET_KEYSPACE, TARGET_USER_NAME, TARGET_PASSWORD);
    ...
    validator.validate(sourceCassandraConnectionProvider, targetCassandraConnectionProvider, sourceTable, targetTable,
        threadsCount);
  }

  private static CassandraConnectionProvider getCassandraConnectionProviderFromConfiguration(String hosts, String port,
    ...
    return new CassandraConnectionProvider(topologyProperties.getProperty(hosts),
        Integer.parseInt(topologyProperties.getProperty(port)), topologyProperties.getProperty(keyspace),
        topologyProperties.getProperty(userName)
