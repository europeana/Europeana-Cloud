...
public class ProviderCleanerTool {
   ...
   * <p>
   * Example parameters for cleaning only records in test mode --test-mode yes --only-records yes --url
   * https://test.ecloud.psnc.pl/api --username admin --password ***** --providers-file /path/to/providers/file/test_providers
   * <p>
   * Example parameters for cleaning only datasets in normal mode --only-datasets yes --url https://test.ecloud.psnc.pl/api
   * --username metis_test --password ***** --providers-file /path/to/providers/file/test_providers
   * <p>
  ...
  private static Options getOptions() {
    ...
    commandLineHelper.addOption(RECORDS_FILE,
        "File with records to remove ids. It can be used instead provider-id or providers-file", false);
    ...
    commandLineHelper.addOption(TEST_MODE,
        "Test mode flag (yes/no). If set list of data to remove will be only displayed - not removed!", false);
