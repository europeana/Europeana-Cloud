...
public class RevisionRemoverJobTest {
  ...
  @InjectMocks
  RevisionRemoverJob revisionRemoverJob = new RevisionRemoverJob(dataSetServiceClient, recordServiceClient, revisionInformation,
  ...
  public void shouldRemoveRevisionsOnly() throws Exception {
    ...
    RevisionInformation revisionInformation = new RevisionInformation("DATASET", DATA_PROVIDER, SOURCE + REPRESENTATION_NAME,
        REVISION_NAME, REVISION_PROVIDER, getUTCDateString(date));
    revisionRemoverJob.setRevisionInformation(revisionInformation);

    Representation representation = testHelper.prepareRepresentation(SOURCE + CLOUD_ID, SOURCE + REPRESENTATION_NAME,
        SOURCE + VERSION, SOURCE_VERSION_URL, DATA_PROVIDER, false, date);
    ...
    when(dataSetServiceClient.getDataSetRevisionsChunk(anyString(), anyString(), anyString(), anyObject(), anyString(),
        anyInt())).thenReturn(resultSlice);
    when(recordServiceClient.getRepresentationsByRevision(SOURCE + CLOUD_ID, SOURCE + REPRESENTATION_NAME,
        new Revision(REVISION_NAME, REVISION_PROVIDER, date))).thenReturn(Arrays.asList(representation));
    when(recordServiceClient.getRepresentationsByRevision(SOURCE + CLOUD_ID2, SOURCE + REPRESENTATION_NAME,
        new Revision(REVISION_NAME, REVISION_PROVIDER, date))).thenReturn(Arrays.asList(representation));
    Thread thread = new Thread(revisionRemoverJob);
    thread.start();
    thread.join();

    verify(revisionServiceClient, times(NUMBER_OF_RESPONSES * NUMBER_OF_REVISIONS)).deleteRevision(anyString(), anyString(),
        anyString(), anyObject());
