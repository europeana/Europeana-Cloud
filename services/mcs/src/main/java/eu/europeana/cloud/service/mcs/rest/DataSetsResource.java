...
public class DataSetsResource {
    ...
    public List<DataSet> getDataSets() throws ProviderNotExistsException {
        return providerService.getDataSets(providerId);
    }

    @POST
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public Response createDataSet(@FormParam(F_DATASET) String dataSetId) throws ProviderNotExistsException, DataSetAlreadyExistsException {
        if (dataSetId == null) {
            return Response.status(Response.Status.BAD_REQUEST).build();
        }
        DataSet dataSet = providerService.createDataSet(providerId, dataSetId);
