...
final class EnrichUriUtil {
    ...
    static void enrich(UriInfo uriInfo, Representation representation) {
        URI allVersionsUri = uriInfo.getBaseUriBuilder().path(RepresentationVersionsResource.class).buildFromMap(ImmutableMap.of(
        ...
        if (representation.getVersion() != null) {
            URI latestVersionUri = uriInfo.getBaseUriBuilder().path(RepresentationVersionResource.class).buildFromMap(ImmutableMap.of(
                    P_GID, representation.getRecordId(),
                    P_SCHEMA, representation.getSchema(),
    ...
    static void enrich(UriInfo uriInfo, String recordId, String schema, String version, File file) {
        URI fileUri = uriInfo.getBaseUriBuilder().path(FileResource.class).buildFromMap(ImmutableMap.of(
                P_GID, recordId,
                P_SCHEMA, schema,
                P_VER, version,
    ...
    static void enrich(UriInfo uriInfo, DataProvider provider) {
        URI providerUri = uriInfo.getBaseUriBuilder().path(DataProviderResource.class).buildFromMap(ImmutableMap.of(
    ...
    static void enrich(UriInfo uriInfo, DataSet dataSet) {
        URI datasetUri = uriInfo.getBaseUriBuilder().path(DataSetResource.class).buildFromMap(ImmutableMap.of(
