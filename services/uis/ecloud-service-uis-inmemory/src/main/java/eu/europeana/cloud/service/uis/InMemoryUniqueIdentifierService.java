...
public class InMemoryUniqueIdentifierService implements UniqueIdentifierService {
     ...
     */
    public InMemoryUniqueIdentifierService(InMemoryCloudIdDao cloudIdDao,
    ...
    public CloudId createCloudId(String... recordInfo) throws DatabaseConnectionException,
        ...
        if (localIdDao.searchActive(providerId, recordId).size() > 0) { throw new RecordExistsException(
                new IdentifierErrorInfo(IdentifierErrorTemplate.RECORD_EXISTS.getHttpCode(),
    ...
    public CloudId getCloudId(String providerId, String recordId)
        ...
        if (cloudIds.size() == 0) { throw new RecordDoesNotExistException(new IdentifierErrorInfo(
    ...
    public List<CloudId> getCloudIdsByProvider(String providerId, String start, int end)
        ...
        if (cloudIds.size() == 0) { throw new RecordDatasetEmptyException(new IdentifierErrorInfo(
    ...
    public void createIdMapping(String cloudId, String providerId, String recordId)
        ...
        if (localIds.size() != 0) { throw new IdHasBeenMappedException(new IdentifierErrorInfo(
                IdentifierErrorTemplate.ID_HAS_BEEN_MAPPED.getHttpCode(),
                IdentifierErrorTemplate.ID_HAS_BEEN_MAPPED.getErrorInfo(providerId, recordId,
                        cloudId))); }
        List<CloudId> cloudIds = cloudIdDao.searchActive(cloudId);
        if (cloudIds.size() == 0) { throw new CloudIdDoesNotExistException(new IdentifierErrorInfo(
                IdentifierErrorTemplate.CLOUDID_DOES_NOT_EXIST.getHttpCode(),
    ...
    @Override
    public void deleteCloudId(String cloudId) throws DatabaseConnectionException,
            CloudIdDoesNotExistException, ProviderDoesNotExistException,
            RecordIdDoesNotExistException {
        if (!(cloudIdDao.searchActive(cloudId).size() > 0)) { throw new CloudIdDoesNotExistException(
        ...
        for (CloudId cId : localIds) {
            ...
            cloudIdDao.delete(cloudId, cId.getLocalId().getProviderId(),
